...
echo "\$(facter -p ipaddress) ${hostname}-${node}-17.${domain} ${hostname}-${node}-17" | tee -a /etc/hosts
sed -i "s/HOSTNAME=.*/HOSTNAME=${hostname}-${node}-17.${domain}/" /etc/sysconfig/network
hostname ${hostname}-${node}-17
...
echo "${aws_userdata}" > $DIR/${hostname}-${node}-17.sh
}
