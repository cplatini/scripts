...
echo '172.16.180.127        puppet-master-01.sys.raxdfw.3mhis.net' | tee -a /etc/hosts
echo '192.168.4.120        yum-bg-30001.sys.aws.3mhis.net' | tee -a /etc/hosts
ntpdate -u 10.18.49.149
curl -k https://puppet-master-01.sys.raxdfw.3mhis.net:8140/packages/current/install.bash | bash -s agent:pluginsignore=.svn
__EOF__
...
aws_instance_id=$(ec2-run-instances ${aws_ami} -t ${flavor} -f $DIR/${hostname}-${node}.sh -a :0:${aws_subnet}:::${aws_group} -b /dev/xvdf=snap-c7532bdf:300 | grep 'INSTANCE' | awk '{ print $2}')
rm $DIR/${hostname}-${node}.sh
sleep 5
## Tag the instance using the ID
ec2-create-tags ${aws_instance_id} --tag "Name=${hostname}-${node}" --tag "Stack=${sdlc}" --tag "Requestor=A3SDAZZ" --tag "Role=Puppet-Compile-Master"
}
