...
s_build_update='[Wed Nov 18 11:37:29 EST 2015 ]' # Change me, if changes are made
s_org='3M HIS, Cloud Systems'
s_author='Nicholas M. Houle, Richard DeHaven, Donald Muwan'; # Add name ", name" when changes are made
s_version='2.5.0'
###########################################
...
# API_NAME='raxca'
# HOST_NAME: t-vm-dmuwan ('t' denotes test VM)
# AVAIL_ZONE: AZ-NP-STANDARD-1, AZ-NP-STANDARD-2
# ENV_NAME: CADEV, CAQA, CACT, CAPROD
# VM_TYPE: NLP, WEB, BATCH, SOLR, LOOKUP, GRYPHON [OS_ID]
...
###########################################
  case "${VM_TYPE}" in
      ...
      NLPD)
        ...
        OS_FLAVOR='496623ee-755c-4efb-84f0-784977dedf3a'
        ;;
      NLPR)
        ...
        ;;
      NLPQ)
        # nlpq_standard - 8 vCPUs, 32G memory, 60G disk, 15G ephemeral
        OS_FLAVOR='7eca4f29-4591-4285-94af-0c409ffe4bf9'
        ;;
      CQLRTR)
        # CQL_RTR - 2 vCPUs, 8G memory, 60G disk, 
        OS_FLAVOR='61'
        ;;
      *)
...
###########################################
  case "${VM_TYPE}" in
      HDD)
        # c4.large - 4 vCPUs, 16G memory, 150G disk, 0 ephemeral
        OS_FLAVOR='0dd08a6b-cb38-48c8-9ede-cf881b85590f'
        ;;
      NLPD)
        # m2.2xlarge - 2 vCPUs, 49G memory, 60G disk, 0 ephemeral
        OS_FLAVOR='ddc02fd6-dd0c-49a2-93f3-5a26785b0893'
        ;;
      NLPR)
        # m2.2xlarge - 2 vCPUs, 49G memory, 60G disk, 0 ephemeral
        OS_FLAVOR='ddc02fd6-dd0c-49a2-93f3-5a26785b0893'
        ;;
      NLPQ)
        # m8.xlarge  - 8 vCPUs, 32G memory, 150G disk, 0 ephemeral
        OS_FLAVOR='57693c61-3ceb-457a-871c-eb6e0f868c83'
        ;;
      NLPKWB)
        # c4.large  - 4 vCPUs, 16G memory, 150G disk, 0 ephemeral
        OS_FLAVOR='0dd08a6b-cb38-48c8-9ede-cf881b85590f'
  ...
  ###########################################
    case "${VM_TYPE}" in
        WEB)
          # c4.large - 4 vCPUs, 16G memory, 150G disk, 0 ephemeral
          OS_FLAVOR='006d73cb-f4b1-4bb9-a6fc-30684a395c82'
          ;;
        NLP)
          # m2.2xlarge - 2 vCPUs, 49G memory, 60G disk, 0 ephemeral
          #OS_FLAVOR='ddc02fd6-dd0c-49a2-93f3-5a26785b0893'
...
###########################################
if [[ "${VM_TYPE}" == "NLPD" ]] || [[ "${VM_TYPE}" == "NLPR" ]] || [[ "${VM_TYPE}" == "NLPQ" ]] || [[ "${VM_TYPE}" == "NLPKWB" ]]; then
...
# DESCRIPTION:
# This is a requirement for NLPQ Deployments.
# Expand LVM Device Mappers for the host
# Rackspace='ssh_raxdfw_make_lvm.part' -or-
# Sungard='ssh_sgden1_make_lvm.part'
###########################################

#if [[ "${VM_TYPE}" == "NLPQ" ]]; then
#  # This is for NLPQ Deployments.
#  if [ -f "${P_ROOT}/bin/ssh_${API_NAME}_make_lvm.part" ]; then
#  echo -e "[INFO]: Running LVM partition sub-routine"
#  # Using API_NAME, configure host for the DC location.
#  ssh ${ssh_string} 'bash /dev/stdin -d vdb -p msdos -m /dev/mapper/vg1-lv_varlog' < "ssh_${API_NAME}_make_lvm.part"
#  fi
#
#fi
